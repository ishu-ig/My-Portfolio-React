...
export default function AdminAchievement() {
  ...
  function updateRecord(_id) {
    ...
    if (!item) return;
    dispatch(updateAchievement({ _id, active: !item.active }));
    setFlag(!flag);
  }
  ...
  const totalCount    = AchievementStateData ? AchievementStateData.length : 0;
  const activeCount   = AchievementStateData ? AchievementStateData.filter((i) => i.active).length : 0;
  const inactiveCount = AchievementStateData ? AchievementStateData.filter((i) => !i.active).length : 0;

  const filteredData = AchievementStateData
    ? AchievementStateData.filter((item) =>
    ...
    <>
      ...
      <main className="dashboard-content">
        <div className="container-fluid px-3 px-lg-4 py-4">
            ...
            <div className="page-heading-copy">
              ...
              <div>
                ...
                <p className="text-muted mb-0">
                  Review and manage achievements.
                </p>
              </div>
            </div>
          ...
          {/* Metric Cards */}
          <section className="row g-3 mt-2 mb-1" aria-label="Achievement summary">
          ...
          <section className="panel mt-3">
            <div className="panel-header">
              <div>
                ...
                <p className="text-muted mb-0">
                  Search, review, and manage achievements.
                </p>
              </div>
            ...
            <div className="table-responsive">
              <table className="table align-middle mb-0" id="achievementsTable">
                <thead>
                  <tr>
                    <th scope="col">#</th>
                  ...
                  {filteredData.length > 0 ? (
                    filteredData.map((item, index) => (
                      <tr key={item._id}>
                        ...
                        <td>
                          <span
                            className={`badge ${item.type === "counter" ? "text-bg-info" : "text-bg-secondary"}`}
                          >
                            {item.type}
                          </span>
                        </td>
                        ...
                        <td className="text-end">
                          <div className="act-strip">
                            <Link
                              ...
                              to={`/achievement/update/${item._id}`}
                              data-tip="Edit"
                            >
                    ...
                    <tr>
                      <td colSpan="9" className="text-center text-muted py-4">
                        {search
                          ? `No achievements found for "${search}"`
