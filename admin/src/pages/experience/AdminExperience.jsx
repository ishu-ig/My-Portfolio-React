...
import {
  ...
  deleteExperience,
  updateExperience,
} from "../../Redux/ActionCreators/ExperienceActionCreators";
...
export default function AdminExperience() {
  ...
  function updateRecord(_id) {
    const item = ExperienceStateData.find((e) => e._id === _id);
    if (!item) return;
    dispatch(updateExperience({ _id, active: !item.active }));
    setFlag(!flag);
  }
  ...
  const totalCount    = ExperienceStateData ? ExperienceStateData.length : 0;
  const activeCount   = ExperienceStateData ? ExperienceStateData.filter((i) => i.active).length : 0;
  const inactiveCount = ExperienceStateData ? ExperienceStateData.filter((i) => !i.active).length : 0;

  const filteredData = ExperienceStateData
    ? ExperienceStateData.filter(
        (item) =>
          ...
          <div className="page-heading">
            <div className="page-heading-copy">
              ...
              <div>
                ...
                <h1 className="h3 mb-1">Experience</h1>
                <p className="text-muted mb-0">
                  Review and manage work experience entries.
                </p>
              </div>
            </div>
            <div className="heading-actions">
              <Link className="btn btn-primary btn-sm" to="/experience/create">
                <i className="bi bi-plus-circle" aria-hidden="true"></i> Add Experience
              </Link>
            </div>
          </div>

          {/* Metric Cards */}
          <section className="row g-3 mt-2 mb-1" aria-label="Experience summary">
            <div className="col-12 col-sm-6 col-xl-4">
              <article className="metric-card text-white">
                ...
                <div className="metric-meta"><span>all</span><span>entries</span></div>
              </article>
            </div>
            <div className="col-12 col-sm-6 col-xl-4">
              <article className="metric-card text-white">
          ...
          <section className="panel mt-3">
            <div className="panel-header">
              <div>
                ...
                <p className="text-muted mb-0">
                  Search, review, and manage work experience entries.
                </p>
              </div>
            ...
            <div className="table-responsive">
              <table className="table align-middle mb-0" id="experienceTable">
                <thead>
                  <tr>
                    <th scope="col">#</th>
                  ...
                  {filteredData.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="7" className="text-center text-muted py-4">
                        {search
                          ? `No experience entries found for "${search}"`
